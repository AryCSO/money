...
# ============================================================
# instalar_money.sh
# Execute no Terminal: bash instalar_money.sh
# ============================================================
...
fail()  { erro "$*"; echo; erro "Corrija o erro e execute novamente."; exit 1; }
...
if [[ "${BUILD_APP}" == "1" ]]; then
  ...
  if [[ -d "/Applications/${MONEY_APP_NAME}.app" ]]; then
    ...
    echo -e "    Execute: ${CYAN}bash ${MONEY_ROOT}/abrir_money.sh${NC}"
  fi
  echo
fi
...
echo -e "${DIM}  Reconfigurar:   bash instalar_money.sh${NC}"
